...
const CACHE_NAME = 'portal-colaborador-v13';
// Guarda só o primeiro nome de quem usa este aparelho, gravado pelo portal no login.
// Serve para a notificação cumprimentar a pessoa sem que o nome trafegue por fora.
const CACHE_PESSOAL = 'portal-pessoal';
...
self.addEventListener('activate', function(event){
  event.waitUntil(
    caches.keys().then(function(chaves){
      return Promise.all(
        chaves.filter(function(k){ return k !== CACHE_NAME && k !== CACHE_PESSOAL; })
...
self.addEventListener('fetch', function(event){
  ...
  // O manifest entra aqui junto com a página: é dele que o celular tira ícone, nome e cores
  // na hora de instalar. Se viesse do cache, a instalação repetiria a versão antiga para
  // sempre, mesmo com o arquivo novo publicado.
  const ehPaginaPrincipal = req.mode === 'navigate' ||
                            req.url.indexOf('index.html') >= 0 ||
                            req.url.indexOf('manifest.json') >= 0;
...
async function nomeDoAparelho(){
  try{
    const cache = await caches.open(CACHE_PESSOAL);
    const resposta = await cache.match('/meu-nome');
    if(!resposta) return '';
...
async function montarAviso(){
  ...
  // Saudação com o nome de quem usa este aparelho, montada aqui no celular.
  const nome = await nomeDoAparelho();
  if(nome) titulo = saudacaoDoMomento()+', '+nome+'!';
  return self.registration.showNotification(titulo, {
    body: corpo,
    icon: './icon-192.png',
    // O Android usa só a silhueta deste arquivo na barra de status, ignorando as cores.
    // Por isso é um desenho simples, branco sobre fundo transparente.
    badge: './badge-96.png',
    tag: 'portal-novidade',
